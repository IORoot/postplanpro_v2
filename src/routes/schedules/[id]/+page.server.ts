...
export const load: PageServerLoad = async ({ params }) => {
	...
	const schedule = db.prepare('SELECT * FROM schedule WHERE id = ?').get(params.id) as {
		id: string;
		name: string;
	...
	`).all() as { id: string; title: string; scheduled_at: string | null; status: string; webhook_name: string }[];

	// Preview next 10 slots from rules (for display)
	let previewSlots: string[] = [];
	if (rules.length > 0) {
		previewSlots = generateSlots(params.id, 10);
	}

	// Posts already attached to this schedule (for reschedule section)
	const schedulePosts = db.prepare(`
		SELECT id, title, scheduled_at FROM post WHERE schedule_id = ? ORDER BY scheduled_at IS NULL, scheduled_at ASC, created_at ASC
	`).all(params.id) as { id: string; title: string; scheduled_at: string | null }[];
...
export const actions: Actions = {
	update: async ({ request, params }) => {
		...
		const db = getDatabase();
		db.prepare('UPDATE schedule SET name = ?, description = ? WHERE id = ?').run(name, description, params.id);
	...
	applySchedule: async ({ request, params }) => {
		const data = await request.formData();
		...
		if (ruleCount.n > 0) {
			slotDatetimes = generateSlots(params.id, postIds.length);
		} else {
		...
		for (let i = 0; i < postIds.length; i++) {
			...
			updatePost.run(slotDatetimes[i], params.id, 'scheduled', postId);
	...
	reschedulePosts: async ({ params }) => {
		const db = getDatabase();
		const ruleCount = db.prepare('SELECT COUNT(*) as n FROM schedule_rule WHERE schedule_id = ?').get(params.id) as { n: number };
		if (ruleCount.n === 0) return fail(400, { error: 'Schedule has no rules. Add rules first, then reschedule.', action: 'reschedule' });

		const posts = db.prepare(`
			SELECT id FROM post WHERE schedule_id = ? ORDER BY scheduled_at IS NULL, scheduled_at ASC, created_at ASC
		...
		const updatePost = db.prepare('UPDATE post SET scheduled_at = ?, updated_at = datetime("now") WHERE id = ?');
		let updated = 0;
		for (const post of posts) {
			const slot = getNextFreeSlot(params.id, post.id);
			if (slot) {
				updatePost.run(slot, post.id);
				updated++;
			} else {
				updatePost.run(null, post.id);
			}
		}
