		...
		{:else if section === 'settings'}
			<section class="mt-6 lg:mt-0" id="account-settings">
				...
				<div class="content-card mt-4 rounded-xl p-6 shadow-sm">
					...
					<div class="mt-4 flex flex-wrap items-center gap-4">
						<p class="text-sm text-[var(--text)]">
							Animations are currently <span class="font-medium">{$showInputAnimations ? 'shown' : 'hidden'}</span>.
						</p>
						<button
							type="button"
							class="inline-flex min-h-[44px] items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-4 py-2.5 text-sm font-medium text-[var(--text)] hover:bg-[var(--surface-hover)]"
							onclick={toggleInputAnimations}
							aria-label={$showInputAnimations ? 'Hide input animations' : 'Show input animations'}
						>
