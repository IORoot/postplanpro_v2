...
/**
 * Seed the test DB with a user (with callback_token) and one webhook_config.
 * Call setTestDatabasePath() and getDatabase() first (e.g. in beforeAll).
 */
export function seedCallbackTestData(): void {
	const db = getDatabase();
	db.prepare(
		"INSERT OR REPLACE INTO user (id, email, email_verified_at, callback_token) VALUES (?, 'test@test.com', datetime('now'), ?)"
	).run(TEST_USER_ID, TEST_CALLBACK_TOKEN);
	db.prepare(
